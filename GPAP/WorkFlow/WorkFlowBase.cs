...
namespace GPAP.WorkFlow
{
    ...
    public class WorkFlowBase
    {
        ...
        public async void WaitComplete()
        {
            if (t != null)
                await t;
        }
        public void UpdateUI(string type, object para)
        {
            switch (type)
            {
                case "Progress":
                    Messenger.Default.Send<int>(Convert.ToInt32(para), type);
                    break;
                case "Collection":
                    Messenger.Default.Send<object>(para, type);
                    break;
                case "FinalResult":
                    Messenger.Default.Send<object>(para, type);
                    break;
